...
class ResumeController extends \yii\web\Controller
{
    ...
    public function actionMinggu()
    {
        ...
        ->one();
        $dataProvider = new SqlDataProvider([
            'sql' => 'SELECT WEEK(tgl) AS minggu, AVG(kelembaban) AS kelembaban, AVG(kecepatan_angin) AS kecepatan_angin,
            (SELECT arah_angin FROM data WHERE id_perangkat="'.$model['id'].'" 
            AND WEEK(tgl)=minggu GROUP BY arah_angin ORDER BY count(arah_angin) DESC LIMIT 1) AS arah_angin,
            AVG(curah_hujan) AS curah_hujan,
            AVG(temperature) AS temperature
            FROM data
            
            WHERE id_perangkat="'.$model['id'].'"
            GROUP BY minggu',
